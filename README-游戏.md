...
<div class="stage">
  ...
  <div id="rulesPanel">
    ...
    <ul>
      ...
      <li>👹 <b>BOSS</b>：巨型坦克，血量厚、三连发炮弹、能直接撞碎砖墙，击毁奖励大量金币</li>
      <li>🎁 <b>随机道具</b>：⭐火力 / 🔧修营地 / 🛡️无敌 / ❄️冻结敌军 / 💣清屏 / 🪙金币，走过去即可拾取</li>
...
const LEVELS=[
  ...
  {total:4, alive:2,mix:['basic','fast'],rate:1.15,boss:true,bossHp:20,bossSp:0.8},   // BOSS ①
  ...
  {total:6, alive:3,mix:['fast','armor'],rate:1.45,boss:true,bossHp:35,bossSp:1.0}    // 最终 BOSS
];
...
const ITEM_TYPES={
  star:  {ico:'⭐',label:'火力 +1'},
  repair:{ico:'🔧',label:'营地修理 +2'},
  shield:{ico:'🛡️',label:'无敌 6 秒'},
  freeze:{ico:'❄️',label:'冻结敌军 5 秒'},
  bomb:  {ico:'💣',label:'全屏爆破'},
  coin:  {ico:'🪙',label:'金币 +80'}
};
...
function setupLevel(){
  ...
  freezeT=0;levelKills=0;itemTimer=600;
...
function drawTank(t,color,flash){
  const s=t.size||(CELL-6);CX.save();CX.translate(t.x+s/2,t.y+s/2);
  CX.rotate({up:0,right:Math.PI/2,down:Math.PI,left:-Math.PI/2}[t.dir]);
  CX.fillStyle=flash?'#ffffff':color;
  CX.fillRect(-s/2,-s/2+s/10,s,s-s/5);
  CX.fillStyle='rgba(0,0,0,.35)';
  CX.fillRect(-s/2,-s/2+s/10,s/6,s-s/5);CX.fillRect(s/2-s/6,-s/2+s/10,s/6,s-s/5);
  CX.fillStyle=flash?'#ffffff':color;
  CX.fillRect(-s/12,-s/2-s/5,s/6,s/2+s/5);
  CX.fillStyle='rgba(255,255,255,.35)';CX.beginPath();CX.arc(0,0,s/5,0,7);CX.fill();
  CX.restore();
}
function draw(){
  CX.save();
  ...
  // 道具
  items.forEach(it=>{
    const blink=it.life<180&&Math.floor(it.life/10)%2===0;
    if(blink)return;
    CX.fillStyle='rgba(0,0,0,.55)';
    CX.beginPath();CX.arc(it.x+12,it.y+12,15,0,7);CX.fill();
    CX.font='19px sans-serif';CX.textAlign='center';
    CX.fillText(ITEM_TYPES[it.type].ico,it.x+12,it.y+19);
  });
  // 营地（小教学楼 + 耐久血条）
  if(baseHp>0){
