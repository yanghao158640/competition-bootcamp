...
<div class="stage">
  ...
  <div id="menuPanel" class="panel">
    ...
    <button class="menuBtn" id="btnSurvival">♾️ 生存模式<span class="tag">最高 20 天 · 生存越久金币越多 · 通关解锁永久 buff</span></button>
  ...
  <div id="rulesPanel" class="panel hide">
    ...
    <ul>
      ...
      <li>♾️ <b>生存模式</b>：最高 20 天，越后越难金币越多；5 天 +200、10 天 +500、20 天 +2000 并解锁永久 buff「黄金徽章」</li>
...
const SURVIVAL_MAX=20;                         // 生存模式最高 20 天
...
function survivalMilestone(w){
  ...
  else if(w===10){bonus=500;msg=' 🏅 生存 10 天奖励 +500'}
  else if(w===20){bonus=2000;msg=' 🏆 生存 20 天！+2000 金币 · 永久「黄金徽章」'}
  if(w===20&&!meta.buff){meta.buff=true;saveMeta()}
  return {bonus,msg};
}
...
function checkLevelEnd(){
  ...
  if(foesTotal===0&&enemies.length===0&&(!boss||boss.dead)){
    if(mode==='campaign'&&level>=CAMPAIGN_LEVELS){
      ...
      const ms=survivalMilestone(20);
      gainCoins(Math.round(ms.bonus*goldMul()));
      winGame('🏆 生存大师！','20 天全部通关 '+ms.msg.trim()+'，敌军闻风丧胆！');
    }
    else onLevelClear();
  }
}
function onLevelClear(){
  ...
  if(mode==='survival'){
    ...
    reward=Math.round((30+level*20+ms.bonus)*goldMul());
    extra=ms.msg;
  }else{
