...
- **12 个关卡 + BOSS 战**：第 4、8、12 关为 BOSS 战（巨型坦克：厚血、三连发散射、
  碾碎砖墙，顶部专属血条，最终 BOSS 54 血）；BOSS 出场即召唤小怪、血量低于 10 时再召唤 5–6 辆增援，需先清场再击毁；每关开始生命重置为 3 条、营地修复 +2；
...
- **更高难度（当前版本）**：敌军开火冷却 55→44（约快 25%）、敌弹速度 5→6（更难躲）、
  ...
  BOSS 血量 20/35/45 → 24/42/54（更耐打）；BOSS 新增「出场召唤小怪 + 残血(≤10)再召唤 5–6 辆增援」，难度显著提升；
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .joystick{position:relative;width:128px;height:128px;border-radius:50%;
    border:2px solid rgba(47,211,151,.55);background:rgba(24,36,32,.5);
    flex:0 0 auto;touch-action:none;user-select:none;-webkit-tap-highlight-color:transparent}
  .joystick::before{content:'';position:absolute;left:50%;top:50%;width:6px;height:6px;
    margin:-3px 0 0 -3px;border-radius:50%;background:rgba(47,211,151,.35)}
  .joystick .stick{position:absolute;left:50%;top:50%;width:48px;height:48px;
    margin:-24px 0 0 -24px;border-radius:50%;
    background:radial-gradient(circle at 35% 30%,rgba(47,211,151,.95),rgba(20,120,90,.92));
    box-shadow:0 0 12px rgba(47,211,151,.55);pointer-events:none}
...
function setupLevel(){
  ...
  if(cfg.boss){
    ...
    boss={x:11*CELL+4,y:CELL,dir:'down',hp:cfg.bossHp,maxHp:cfg.bossHp,
      size:2*CELL-8,sp:cfg.bossSp,cool:80,ai:0,hitT:0,stuck:0,dead:false,summoned2:false};
    S.boss();
  }
  player.x=3*CELL;player.y=12*CELL;player.dir='up';player.invT=90;player.hp=player.maxHp;
  player.dmg=TANK_TYPES[player.type].dmg+mods.dmg;          // 清掉本关拾取的火力道具，不跨关
  player.shieldT=0;rapidT=0;freezeT=0;                      // 护盾/速射/冻结均只持续本关
  if(boss&&!boss.dead){let _s=0,_t=0;while(_s<3&&_t<14){const b=enemies.length;spawnMinion();if(enemies.length>b)_s++;_t++}}  // BOSS 出场召唤小怪
...
function applyItem(type){
  ...
  switch(type){
    case 'star':{const base=TANK_TYPES[player.type].dmg+mods.dmg;if(player.dmg<base+3)player.dmg++;break;}
...
function spawnMinion(){
  ...
  const cfg=levelCfg();
  const mix=(cfg.mix&&cfg.mix.length)?cfg.mix:['basic','fast','armor'];
  const gi=Math.floor(Math.random()*GATES.length);
  const [c,r]=GATES[gi];
  const x=c*CELL+3,y=r*CELL+3;
  if(rectHitsWall(x,y,CELL-6))return;
...
function bossThink(){
  ...
  if(!boss.summoned2 && boss.hp<=10){
    boss.summoned2=true;
    const n=5+Math.floor(Math.random()*2);
    let _s=0,_t=0;while(_s<n&&_t<n*3){const b=enemies.length;spawnMinion();if(enemies.length>b)_s++;_t++}
    if(_s>0)addFloat(boss.x+boss.size/2,boss.y+boss.size+10,'召唤增援 ×'+_s,'#ff7a6e');
  }
  if(--boss.ai<=0){
    boss.ai=30;
...
/* ---------- 固定摇杆：轮盘不随拇指移动，仅按触摸点相对圆心的方向驱动 ---------- */
(function(){
  const joy=document.getElementById('joy');
  if(!joy||typeof joy.querySelector!=='function')return;
  const stick=joy.querySelector('.stick');
  if(!stick)return;
  const R=38, DEAD=10;            // 摇杆最大位移 / 死区（减弱僵硬感：更大行程 + 更小死区）
  let activeId=null;
  const center=()=>{const r=joy.getBoundingClientRect();return{x:r.left+r.width/2,y:r.top+r.height/2}};
  const findTouch=e=>{
    if(!e.touches&&!e.changedTouches)return e;          // 鼠标
    for(const list of [e.touches,e.changedTouches]){
      if(!list)continue;
      for(const t of list)if(t.identifier===activeId)return t;
    }
    return null;
  };
  const dirFromVec=(dx,dy)=>{
    const r={up:0,down:0,left:0,right:0};
    if(Math.hypot(dx,dy)<DEAD)return r;
    const a=(Math.atan2(dy,dx)*180/Math.PI+360)%360;   // 0=右, 90=下
    if(a>=337.5||a<22.5)r.right=1;
    else if(a<67.5){r.right=1;r.down=1;}
    else if(a<112.5)r.down=1;
    else if(a<157.5){r.down=1;r.left=1;}
    else if(a<202.5)r.left=1;
    else if(a<247.5){r.left=1;r.up=1;}
    else if(a<292.5)r.up=1;
    else {r.up=1;r.right=1;}
    return r;
  };
  const onStart=e=>{
    e.preventDefault();
    activeId = e.changedTouches ? e.changedTouches[0].identifier : 'mouse';
    onMove(e);
  };
  const onMove=e=>{
    ...
    keys.up=d.up;keys.down=d.down;keys.left=d.left;keys.right=d.right;
  };
  const onEnd=e=>{
    if(activeId===null)return;
    if(e.changedTouches){
      let hit=false;
      for(const ct of e.changedTouches)if(ct.identifier===activeId){hit=true;break;}
      if(!hit)return;
    }
    activeId=null;
