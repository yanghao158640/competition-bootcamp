...
```
├── tank-game.html    坦克大战（单文件 Canvas 实现，双击即可离线运行）
...
- **开局规则确认**：先展示完整规则（目标/耐久/血量/操作/敌军行为），玩家确认后才开局；
- **自选坦克**：轻型·猎豹（高机动）/ 标准·战狼（均衡）/ 重型·堡垒（双倍伤害）；
- **5 个关卡**：敌军数量、兵种（普通/快速/装甲）与地图障碍逐关递增；
  每关开始生命重置为 3 条、营地修复 +2；
- **敌军 AI 进攻营地**：沿距离场寻路推进、炮轰挡路砖墙开路、
  与营地或玩家对齐时优先开火；
- **营地 7 发耐久**：血条 + ❤ 实时显示，归零即失败；我方每命 3 发血量、3 条生命；
- **打击感**：屏幕震动、枪口火光、爆炸冲击环、命中白闪、击毁飘分、
  WebAudio 合成音效（零素材）；
- 【身份标识】地图中央砖墙摆成姓名缩写「YYH」+ 画布右上角「杨豫豪 · YYH」水印；
...
| `LEVELS / LEVEL_WALLS` | 5 关敌军配置与每关障碍布局 |
| `buildMap()` + `GLYPH` | 地图构建；中央砖墙按字形矩阵摆出「YYH」 |
| `computeDist()` | 以营地为源点的距离场（Dijkstra，砖墙代价 6），供敌军寻路 |
| `enemyThink()` | 敌军 AI：沿距离场进攻营地、拆墙开路、对齐优先开火 |
| `step()` | 主循环：移动、炮弹、碰撞、营地耐久、胜负判定 |
| `draw()` | 绘制：底图、墙体、社区标识、坦克、特效、HUD 遮罩 |
| `S.*` | WebAudio 合成音效（开火/爆炸/受击/胜负） |
| `rulesPanel / selectPanel` | 规则确认面板、坦克选择面板 |
